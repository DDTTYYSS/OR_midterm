...
def solve_instance(N, T, D, C_P, C_V1, C_V2, I_0, I_1, I_2, V, C_C, C_H, C_F, T_lead):
    ...
    purchasing_cost = gp.quicksum(C_P[i] * x[i, j, t] for i in S_I for j in S_J for t in S_T)
    variable_shipping_cost = gp.quicksum(
        C_V1[i] * x[i, 0, t] + C_V2[i] * x[i, 1, t]
        for i in S_I for t in S_T
    )
    fixed_shipping_cost = gp.quicksum(C_F[j] * y[j, t] for j in S_J for t in S_T)
    container_cost = gp.quicksum(C_C * z[t] for t in S_T)

    model.setObjective(holding_cost + purchasing_cost + variable_shipping_cost + fixed_shipping_cost + container_cost, GRB.MINIMIZE)
    ...
    if model.status == GRB.OPTIMAL:
        ...
        total_holding_cost = sum(C_H[i] * v[i, t].x for i in S_I for t in S_T)
        total_purchasing_cost = sum(C_P[i] * x[i, j, t].x for i in S_I for j in S_J for t in S_T)
        total_variable_shipping_cost = sum(
            C_V1[i] * x[i, 0, t].x + C_V2[i] * x[i, 1, t].x
            for i in S_I for t in S_T
        )
        total_fixed_shipping_cost = sum(C_F[j] * y[j, t].x for j in S_J for t in S_T)
        total_container_cost = sum(C_C * z[t].x for t in S_T)
        ...
        # Print shipping costs for each period
        print("\nShipping Costs by Period:")
        for t in S_T:
            period_variable_cost = sum(
                C_V1[i] * x[i, 0, t].x + C_V2[i] * x[i, 1, t].x
                for i in S_I
            )
            period_fixed_cost = sum(C_F[j] * y[j, t].x for j in S_J)
            period_container_cost = C_C * z[t].x if z[t].x > 0 else 0
            
            print(f"\nPeriod {t+1}:")
            print(f"  Variable Shipping Cost: {period_variable_cost:.2f}")
            print(f"  Fixed Shipping Cost: {period_fixed_cost:.2f}")
            print(f"  Container Cost: {period_container_cost:.2f}")
            print(f"  Total Period Shipping Cost: {period_variable_cost + period_fixed_cost + period_container_cost:.2f}")
            
            # Print shipping method usage for this period
            print("  Shipping Method Usage:")
            for j in S_J:
                if y[j, t].x > 0:
                    method_name = "Ocean" if j == 0 else "Air" if j == 1 else "Express"
                    print(f"    {method_name}: {y[j, t].x:.2f}")
            
            # Print order quantities for this period
            print("  Order Quantities:")
            for i in S_I:
                for j in S_J:
                    if x[i, j, t].x > 0:
                        method_name = "Ocean" if j == 0 else "Air" if j == 1 else "Express"
                        print(f"    Product {i+1} - {method_name}: {x[i, j, t].x:.2f}")
